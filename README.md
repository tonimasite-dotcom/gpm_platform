...
>
> Текущий production на 11.09.2026: backend и frontend `c0f0b53`.
> Батч 4 (11.09): сервер сам присваивает номера заказов (`APP-NNNNNN`,
> счётчик в БД вместо клиентской генерации), убран баннер «Единый рабочий
> чат…» внутри переписки, исправлена метка гражданства исполнителя
> («Только РФ» / «Только не РФ» / «Любое» вместо «РФ» / «Не РФ» / «Не
> важно») — run 34605651389, success. Поверх релизов 08.09 выпущены три
> батча правок по кабинетам (аудит клиента/логиста/исполнителя): общий
> модуль статусов/дат (`lib/utils/order_display.dart`), фиксы в списках и
> деталях заказов, дата заказа у клиента, форма создания (гражданство
> «Только РФ / Любое», обязательное «Описание работ», два поля веса),
> разбивка сумм в финансах исполнителя, причёсанная «Главная»
> исполнителя, начисление от ставки исполнителя, удалена плашка в списке
> чатов (`10be0c4`, `d4c253b`, `3dfb53b`, `a4a3dd6`; runs 34345268272 /
> 34349468299 / 34355617294, все success).
> Ветки `feature/crm-logist-city-visibility` и
...
> ещё не настроен.
> Сессия 18.09 (`7692d1e`, задеплоено на backend, run 35336019717):
> обнаружено, что часть заявок всё ещё создаётся со старым форматом номера
> из-за неактуального APK на чьём-то телефоне — сервер раньше слепо
> доверял присланному клиентом номеру. Вместо простого фикса введена новая
> схема нумерации: каждый клиент/логист получает постоянный код при
> регистрации (`L1`, `C1`…), заказы нумеруются `{код}-ДДММГГ-{n}` (счётчик
> за день на актора). Сервер теперь всегда сам генерирует номер, клиентский
> игнорируется. Бэкфилл существующих аккаунтов встроен в
> `deploy-production.yml` (выполняется на каждом деплое бэкенда,
> идемпотентно). 58 backend + 36 Flutter тестов зелёные. Frontend не
> менялся, остаётся на `c0f0b53`.
> `gh` CLI установлен и авторизован на этой машине для самостоятельного
> запуска деплоя. Главный актуальный источник истины:
> `PROJECT_HANDOFF_2026-09-18_ACTOR_ORDER_NUMBERING.md`.
